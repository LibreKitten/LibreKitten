...
const Scratch = require('../../extension-support/tw-extension-api-common.js');
...
/**
 * lk: Blocks to provide the front-end for LibreKitten server support.
 * @constructor
 */
class Server {
    constructor (runtime) {
        ...
        if (!this.runtime.isPackaged) {
            (() => {
                ...
                button.style.flexGrow = 'none';
                button.addEventListener('click', () => {
                    this.runtime.emit('serverRequest', '/', '127.0.0.1', 'GET', JSON.stringify({'User-Agent': 'LibreKittenFakeBrowser/0.0.0'}), '');
                });
                container.appendChild(button);
                const iframe = document.createElement('iframe');
                iframe.style.flexGrow = 1;
                container.appendChild(iframe);
                this.renderer.addOverlay(container, 'scale');

                this.runtime.on('serverResponse', (content, mime, status, extraHeaders) => {
                    iframe.src = `data:${mime};base64,${btoa(content)}`;
                });
            })();
        }

        this.runtime.on('serverRequest', (page, ip, method, headers, data) => {
            this.fourZeroFour = false;
            this.url = page;
            this.ip = ip;
        ...
        this.runtime.on('server404', (page, ip, method, headers, data) => {
    ...
    getInfo () {
        return {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    arguments: {
                        CONTENT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'Hello LibreKitten!'
                        },
                        MIME: {
                            type: ArgumentType.STRING,
            ...
            menus: {
                MIME_MENU: {
                    items: ['text/plain', 'text/html', 'text/css', 'text/javascript', 'application/xml', 'application/json'],
