...
test('internal extension', t => {
    ...
    // lk: Since there are "core extensions" in LibreKitten, we ignore them in this test.
    const defaultLength = vm.runtime._blockInfo.length;
    ...
    t.equal(vm.runtime._blockInfo[defaultLength].menus.length, 2);
    // First menu has 3 items.
    t.equal(
        vm.runtime._blockInfo[defaultLength].menus[0].json.args0[0].options.length, 3);
    // Second menu is a dynamic menu and therefore should be a function.
    t.type(
        vm.runtime._blockInfo[defaultLength].menus[1].json.args0[0].options, 'function');
...
test('load sync', t => {
    ...
    t.equal(vm.runtime._blockInfo.length - defaultLength, 1);

    // blocks should be an array of two items: a button pseudo-block and a reporter block.
    t.equal(vm.runtime._blockInfo[defaultLength].blocks.length, 3);
    t.type(vm.runtime._blockInfo[defaultLength].blocks[0].info, 'object');
    t.type(vm.runtime._blockInfo[defaultLength].blocks[0].info.func, 'MAKE_A_VARIABLE');
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[0].info.blockType, 'button');
    t.type(vm.runtime._blockInfo[defaultLength].blocks[1].info, 'object');
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[1].info.opcode, 'exampleOpcode');
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[1].info.blockType, 'reporter');
    t.type(vm.runtime._blockInfo[defaultLength].blocks[2].info, 'object');
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[2].info.opcode, 'exampleWithInlineImage');
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[2].info.blockType, 'command');

    // Test the opcode function
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[1].info.func(), 'no stage yet');
    ...
    t.equal(vm.runtime._blockInfo[defaultLength].blocks[1].info.func(), 'Stage');
